...
function showSyncAwareResult_(res, successMessage, warningMessage) {
  ...
  if (hasWarning) {
    showMsg_(String(successMessage || "Operacion completada.") + "\nAdvertencia: " + String(res.warning || ""));
    return;
  }
...
async function postApi_(payload) {
  ...
  if (!body.session_token) {
    ...
    if (session && session.session_token) body.session_token = session.session_token;
  }
...
function resetDoctorForm_(mode, doctor) {
  ...
  document.getElementById("doctorModalTitle").textContent = isCreate ? "Nuevo médico" : "Editar médico";
  document.getElementById("btnSaveDoctor").textContent = isCreate ? "Crear médico" : "Guardar cambios";
...
function renderDoctors_() {
  ...
  tbody.innerHTML = "";
  if (!saDoctors.length) {
    tbody.innerHTML = '<tr><td colspan="6" class="text-center">Sin médicos</td></tr>';
    return;
  }
...
async function loadServicesAdmin() {
  ...
  const res = (window.vfDataBridge && window.vfDataBridge.getServices)
    ? await window.vfDataBridge.getServices(requester)
    : await postApi_({ action: "get_services", requester: requester });
  if (!res.success) {
...
async function deleteDoctor_(idx) {
  ...
  const ok = await confirmAction_(
    "Borrar médico",
    `Se eliminará al médico ${doctor.nombre_doctor} (${doctor.usuario}).\nSus pacientes quedarán sin médico asignado (${pacienteCount}).`,
    "Borrar"
  );
  if (!ok) return;
  ...
  if (!res.success) {
    showMsg_(res.message || "No se pudo borrar el médico.");
    return;
  }

  await loadData_();
  showSyncAwareResult_(
    res,
    `Médico eliminado. Pacientes sin asignar: ${res.unassigned_patients || 0}.`,
    "Médico eliminado con advertencia de sincronización."
  );
}
...
async function loadData_() {
  ...
  if (!res.success) {
    showMsg_(res.message || "No se pudo cargar información.");
    return;
  }
...
async function saveDoctor_(ev) {
  ...
  if (!payload.nombre_doctor || !payload.usuario || !payload.password) {
    showMsg_("Completa nombre, usuario y contraseña.");
    return;
  }

  const ok = await confirmAction_(
    mode === "create" ? "Crear médico" : "Guardar cambios",
    mode === "create"
      ? `Se creará el médico ${payload.nombre_doctor}.`
  ...
  try {
    ...
    showSyncAwareResult_(
      res,
      mode === "create" ? "Médico creado." : "Médico actualizado.",
      mode === "create"
        ? "Médico creado con advertencia de sincronización."
        : "Médico actualizado con advertencia de sincronización."
    );
  } finally {
    btn.disabled = false;
...
async function savePatientManagement_(ev) {
  ...
  const ok = await confirmAction_(
    "Modificar paciente",
    "Se guardarán los cambios del médico tratante y la contraseña (si fue escrita).",
    "Guardar"
  );
  if (!ok) return;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const welcome = `Sesión: ${requester}`;
  ...
  document.getElementById("btnLogout").addEventListener("click", async () => {
    const ok = await confirmAction_("Cerrar sesión", "¿Deseas cerrar sesión del panel superadmin?", "Cerrar sesión");
    if (!ok) return;
